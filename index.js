...
app.get("/transfer",function(req,res){
  AccountDetail.find({},function(err,result){
    res.render("transfer",{subhead:"hide",submssg:"",data:result})
  })
  ...
  AccountDetail.find({Acct:req.body.senderId},function(err,result){
    AccountDetail.find({Acct:req.body.recepientId},function(err1,result2){
      ...
      if(result.lenght!=0 && result2.length!=0 && result[0].Balance>=req.body.transferAmt){
          ...
          var1.save();
          AccountDetail.find({},function(err,result){
            res.render("transfer",{subhead:"Success",submssg:"Transaction Success",data:result})
          })
        ...
        var1.save();
        AccountDetail.find({},function(err,result){
          res.render("transfer",{subhead:"error",submssg:"Transaction Failure",data:result})
        })
